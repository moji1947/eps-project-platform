...
import { AlertTriangle, ArrowLeft, Building2, Camera, CheckCircle2, ClipboardCheck, Clock3, ExternalLink, FileText, HardHat, ImagePlus, Info, Link2, MapPin, PackageCheck, Plus, Save, ShieldCheck, TestTube2, Users, Wrench, X } from 'lucide-react'
import './detail.css'
...
export default function ProjectQualityDetail({project,StatusBadge}){
  ...
  const submitIssue=e=>{e.preventDefault();if(!form.zone||!form.title||!form.reference){setNotice('Complete zone, issue title and document reference.');return}const item={id:'AUD-'+String(240+issues.length+1).padStart(4,'0'),date:form.date.replace('T',' '),zone:form.zone,title:form.title,status:form.status,reference:form.reference,before:photos.before.length,after:photos.after.length};setIssues(list=>[item,...list]);setForm({...form,zone:'',title:'',description:'',reference:''});setPhotos({before:[],after:[]});setIssueFormOpen(false);setNotice('Site issue saved to this browser.')}
  const props={project,StatusBadge,showMethodology,setShowMethodology,notice,setNotice,tests,addTest,updateTest,issues,form,setForm,photos,attachPhotos,submitIssue,issueFormOpen,setIssueFormOpen}
  return <DetailDashboard {...props}/>
}

function DetailDashboard({project:p,StatusBadge,showMethodology,setShowMethodology,notice,setNotice,tests,addTest,updateTest,issues,form,setForm,photos,attachPhotos,submitIssue,issueFormOpen,setIssueFormOpen}){
  ...
  const disciplines=[['Civil','Excellent'],['Structure',p.openNcrCritical?'Critical':'Good'],['Mechanical',p.reworkPercent>2?'Risk':'Good'],['Piping','Excellent'],['Electrical',p.holdPointBypassed?'Critical':'Good'],['Boiler','Good'],['Commissioning',p.punchListOverdue>5?'Risk':'Excellent']]
  ...
  return <div className="quality-detail">
    ...
    <main className="quality-detail__main"><section className="detail-kpis" aria-label="Target-driven project KPIs">{kpis.map(([label,value,target])=><DetailKpi key={label} label={label} value={value} target={target}/>)}</section>
      <div className="detail-workspace">
        ...
        <IssuePanel issues={issues} form={form} setForm={setForm} photos={photos} attachPhotos={attachPhotos} submitIssue={submitIssue} open={issueFormOpen} setOpen={setIssueFormOpen}/>
